...
file.  To read events from an archived `.evtx` file you can specify
the `name` as the absolute path (it cannot be relative) to the file.
...
include::input-shared-settings.asciidoc[tag=index-setting]
