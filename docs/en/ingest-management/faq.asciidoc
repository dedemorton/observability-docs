...
{ingest-manager}, {kib} connects to the {package-registry} at
+epr-{minor-version}.elastic.co+, downloads the integration package, and stores
its assets in {es}. This means that you no longer have to run a manual setup
command to load integrations as you did previously with {beats} modules.
...
downloads from the {package-registry} at +epr-{minor-version}.elastic.co+. This
means that {agent} does not need extra privileges to set up assets because
